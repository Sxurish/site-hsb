  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          <section className="lg:col-span-2 rounded-2xl border border-border bg-surface p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 border-t border-border pt-4">
              {[
                { label: 'Total',  value: loading ? '—' : String(leadStats.total) },
                { label: 'Média',  value: loading ? '—' : `${leadStats.avgPerDay}/d` },
                { label: 'Melhor', value: loading ? '—' : String(leadStats.best) },
              ].map((s) => (
        ...
        {/* Bottom row: funil resumido + top serviços */}
            ...
            <div className="space-y-3">
              {loading
                ...
                : funnelSummary.length === 0
                  ? <p className="text-xs text-muted">Sem dados de funil no período.</p>
                  : funnelSummary.map((row) => {
                      const color = row.warn ? '#f87171' : '#d4a566';
                      return (
                        <div key={row.label}>
                          <div className="mb-1 flex justify-between text-xs">
                            <span className="text-muted">{row.label}</span>
                            <span className="font-mono font-bold" style={{ color }}>{row.pct}%</span>
                          </div>
                          <div className="h-1 rounded-full bg-surface-2">
                            <div
                              className="h-full rounded-full"
                              style={{ width: `${Math.min(row.pct, 100)}%`, background: color, opacity: row.warn ? 1 : 0.7 }}
                            />
                          </div>
                        </div>
                      );
            ...
            <div className="space-y-3">
              {loading
                ...
                : topServices.length === 0
                  ? <p className="text-xs text-muted">Nenhum lead com serviço definido no período.</p>
